# Authorship and provenance

This document was written by large language models (primarily Claude, with some work by OpenAI's Codex GPT-5.5) under human
direction, with machine-verified audits for the finite claims and
independent-session cross-verification for the theoretical ones; one
conditional theorem announced during development (a conditional infinite
family on the lane) was retracted after verification and does not appear in
this document. The working record, including the retraction, is preserved
in the companion notes.

This repository is shared for open iteration rather than journal
submission. Before external citation, run the adversarial claim review
described in STATUS.md; contributions that kill, strengthen, or simplify
any claim are welcome.
