...
\begin{abstract}
Turyn's 1972 construction produces a Hadamard matrix of order $4t$ from a pair
of symmetric circulants $R,S$ of odd order $t$ with $R^2+S^2=(2t-1)I$, whenever
$q=2t-1$ is a prime power; Turyn conjectured that these pairs exist \emph{only}
when $q$ is a prime power. This note assembles, in one place and with uniform
nomenclature, a program of exact number-theoretic obstructions aimed at that
converse. The hierarchy starts with the two-squares test, the exact boundary of
the classical self-conjugacy obstruction, multiplier theorems with exact signs,
and a prime-quotient descent governed by the equation
$\alpha^2+\beta^2=q$ in $\OK=\Z[\zeta_m+\zeta_m^{-1}]$. It then adds
field-free full-torus and sub-torus marginal decisions from the composite
multiplier theorem, including the signed marginal module for $\epsilon=-1$
branches, exact full-torus sign joins, and secondary exact marginal checks. We
prove an unconditional rigidity criterion that forces every prime-quotient
solution to be rational when the prime divisors of $q$ are \emph{rigid} for
$m$, and we decompose the remaining nonrigid prime-quotient case into three
exact conditions on a Weil generator $\gamma\in\OL=K_m(i)$: a norm equation, a
ray-class congruence modulo $2p_m$, and an integral realization within a
coefficient box. On top of the hierarchy we prove the selection layer of
the divisor program: marginal feasibility is monotone on the divisor
lattice of $t$, the orbit model at every divisor is computed by the
multiplier group $M(q)$ alone, the full-torus node is exact, and
nonexistence at $q$ is \emph{equivalent} to emptiness of some divisor
node---so the open content of the converse is isolated in per-family
firing theorems.  Several of these are given in closed form: a
representation-pair criterion deciding every transitive two-orbit
marginal, an $h=3$ support/augmentation hierarchy that routes the
dominant local-pass class, the full-unit prime-power nested criterion,
and a row-sum bound at prime-$t$ top nodes---sharp exactly at Turyn's
$q=25$---whose contrapositive bounds the multiplier image of any genuine
prime-order pair by $w<1+\sqrt t$, together with its exact sub-threshold
form.  The field-free hierarchy closes every self-conjugacy-blind
multi-prime composite $q\le2000$ by written quantified firing criteria
($81$ proper-divisor fires, $29$ full-torus fires, and the
projection-gluing case $q=441$; no finite certificate remains),
including the apparently first determinations $q=1469$, $q=1937$, and
$q=1325$ (first in the negacyclic/Turyn-pair literature searched in the
2026-07-06 audit; a closed-table caveat remains).
Beyond the field-free ledger, the small-image top nodes ($w\le d+1$)
are attacked by a fold-tower/stabilizer-amplification dichotomy whose
single conditional input is one class-group computation: it yields the
further (GRH-conditional) first determinations $q=5185$ and $q=62305$,
two of the four exact-predicate census survivors at $q\le10^5$.  A
companion structural result---the uniform character-lifting normal form
and its fillability dichotomy, with arithmetic specializations---is
included on the construction side. On the lane $q=6\ell-1$ with
$\ell\equiv3\bmod4$ prime, the criteria fire \emph{deterministically}
at every composite $q$ with two prime factors, giving the program's
first infinite-family-shaped partial converse: unconditional at each
member, infinite under Dickson's conjecture, and --- under a
congruence-restricted Chen input stated as an explicit hypothesis ---
``either $6\ell-1$ is prime infinitely often, or infinitely many
composite $q$ admit no Turyn pair''; full unconditional infinitude is
precisely a parity-problem instance. At general $\omega$ the lane
collapse persists --- a Turyn pair at composite $q=6\ell-1$ forces the
multiplier image in $(\Z/\ell)^\times$ down to $\{\pm1\}$ --- and a
character-pinning identity determines the residue status of the one
prime factor above $\sqrt q$ from the small factors; under a single
named Bombieri--Vinogradov-type hypothesis for cubic Kummer splitting
(implied by GRH), at least $c\,x/(\log x)^{4/3}$ sub-lane primes have
every small prime factor of $q$ obstructing, with the passage to an
infinite family of certified nonexistence orders isolated in one
remaining open structure count above the sieve level. The contribution is neither a proof
of the converse nor a Hadamard nonexistence result: it is a theorem
roadmap whose remaining obligations are a Stickelberger-type
description of the fold firing condition, lattice-reduced high-degree
period joins, composite-$t$ top nodes and a general gluing theorem, an
unconditional averaged Kummer equidistribution theorem with its
above-sieve-level companion for the lane density program, and
the structural routing argument that some criterion always fires---by
the selection theorem, that statement is the converse itself.
\end{abstract}
...
\subsection{What this note does, and the meaning of ``closing''}
\label{sec:contribution}

This note assembles a program of \emph{exact necessary conditions} on a Turyn
pair. Each is an obstruction: if it fails for a given composite $q$, no Turyn
pair of order $t=(q+1)/2$ exists, and that value of $q$ is \emph{closed} as a
case of Conjecture~\ref{conj:converse}. Three points about the nature of this
contribution.

\begin{enumerate}
\item \emph{Closing a case of the converse is not a Hadamard nonexistence
  result.} Proving that no Turyn \emph{pair} of order $t$ exists does not prove
  that no Hadamard matrix of order $4t$ exists; such a matrix may well be
  produced by an unrelated construction. We rule out one structured route, and
  thereby confirm the converse at that order---nothing more.
\item \emph{It is evidence for the conjecture, order by order, not a proof of
  it.} The obstructions are necessary conditions; passing all of them leaves the
  pair's existence open (and, for the smooth composite orders of
  Section~\ref{sec:gluing}, genuinely undecided by any method here).
\item \emph{Epistemic status of specific closed orders.} For small $q$ the
  nonexistence of a Turyn pair is already a data point in the $v<1000$
  negacyclic classification~\cite{DGS71,Seberry16}; there the contribution is a
  \emph{proof} in the Turyn-pair normal form, not a first determination. For
  larger $q$ outside that range the closure may be genuinely first.
\end{enumerate}

The program is a descending hierarchy (Section~\ref{sec:hierarchy}): trivial
character (\S\ref{sec:twosquares}), self-conjugacy (\S\ref{sec:selfconj}),
multiplier symmetry (\S\ref{sec:multiplier}), the composite-multiplier reduced
decision (\S\ref{sec:t3-decision}), sub-torus marginal obstruction
(\S\ref{sec:t2-marginal}), quotient descent and the rigidity criterion
(\S\ref{sec:rigidity}), and the nonrigid three-condition analysis
(\S\S\ref{sec:nonrigid}--\ref{sec:defect}). Section~\ref{sec:mary} records the
construction-side companion in full. Section~\ref{sec:results} tabulates the
closed orders; Sections~\ref{sec:related}--\ref{sec:remains} give context and
the road ahead.
...
beyond two squares, and $13$ further $q\le2000$.
...
divisor $\ell\equiv3\pmod4$ in a suitable position. Every composite $q$ all of
whose prime divisors are $\equiv1\pmod4$ is therefore invisible to the entire
self-conjugacy class---e.g.\ $q=65=5\cdot13$ ($t=33$), for which no inert prime
appears in any $\Q(\zeta_{4d})$ layer. There are $111$ such composite $q\le2000$;
they are the domain of Sections~\ref{sec:t3-decision}--\ref{sec:defect}. (Exact
quotient descent nonetheless closes $q=65$ via $\Z[C_{33}]\to\Z[C_{11}]$; the
T3 and quotient mechanisms developed next attack this same blind spot by
different means.)
\end{remark}
...
$\varphi(p^e)/\varphi(p^{m-j})\cdot\mu(p^{m-j})$ for $m\ge j$ and
...
\begin{corollary}[$C_{49}$ full-unit nested criterion]\label{prop:c49-nested}
...
\texttt{condition\_r\_realization.py} and cached as
\texttt{condition\_r\_realization\_qmax2000\_degree40.jsonl}.
...
Condition (C) holds for $(m,q)$ iff there are $I\in C_{\mathrm{loc}}(m,q)$,
$r\in T_B(m,q)$, $\mu\in\mu_4$, $\sigma\in\{1,c\}$ with $I$ unit-admissible and
$\operatorname{cl}_B(\sigma I)=[\mu^{-1}r]_B$ in $\Cl_B(L)$. Equivalently, (C) is
decided by local split-prime choices together with the single global image
$D_B(m,q)$.
\end{proposition}
...
$\mu\sigma(\gamma)\equiv r\pmod{M_B}$, which says $\sigma I$ has a generator
$\equiv\mu^{-1}r$, i.e.\ the displayed ray-class equality.
\end{proof}
...
\begin{theorem}[Absorbed ray criterion]\label{thm:absorbedC}
Let $G_B=(\OL/M_B)^\times$. Let $\Lambda_B\subseteq G_B$ be the subgroup
generated by the following field-constant ambiguities: the residues of $\mu_4$;
the local Hilbert--90 norm-one residues $z/c(z)$ with $z\in G_B$; and the
residues of the global relative-unit corrections $u/c(u)$ with
$u\in\OL^\times$. For each unit-admissible $I\in C_{\mathrm{loc}}(m,q)$ choose
one generator $\gamma_I$ with $\gamma_Ic(\gamma_I)=q$, and put
\[
  \overline S_B(m,q)=
  \{\sigma(\gamma_I)\Lambda_B:\ I\in C_{\mathrm{loc}}(m,q)\text{ unit-admissible},
      \ \sigma\in\{1,c\}\}\subseteq G_B/\Lambda_B.
\]
Let
\[
  \overline T_B(m,q)=\{r\Lambda_B:\ r\in T_B(m,q)\}.
\]
Then condition \emph{(C)} holds if and only if
\[
  \overline S_B(m,q)\cap\overline T_B(m,q)\ne\varnothing.
\]
Equivalently, the only global input needed by the congruence condition is the
finite absorbed image $\overline D_B(m,q):=\overline S_B(m,q)$ of the
unit-admissible ray defect.
\end{theorem}

\begin{proof}
Theorem~\ref{thm:descent} and Lemma~\ref{lem:raygluing} identify (C) with the
existence of an oriented conjugate of a norm-$q$ generator whose residue lies in
$T_B(m,q)$. By Lemma~\ref{lem:localfactor}, the ideal of such a generator is an
element of $C_{\mathrm{loc}}(m,q)$, and by Proposition~\ref{prop:fieldfree} the
only obstruction to reading its residue as a local product is the ray class of
that ideal. Changing the principal local Hilbert--90 choices, the relative-unit
correction in the norm equation, or the fourth-root orientation multiplies the
residue by an element of $\Lambda_B$; conversely, the generators of
$\Lambda_B$ are precisely these absorbed choices. Thus a unit-admissible local
ideal contributes one well-defined coset $\sigma(\gamma_I)\Lambda_B$ in
$G_B/\Lambda_B$, independent of the auxiliary generator choices. The displayed
intersection is exactly the projected source condition
$S_B(m,q)\cap T_B(m,q)\ne\varnothing$.
\end{proof}
...
The following table, from the field-tractable panel, exhibits the absorbed
congruence quotient. Here $|\overline D_B|$ is the number of residual source
cosets in the target-relevant quotient, ``hit'' is the number meeting
$\overline T_B$, and ``cert.'' records whether the class data for $K_m$ and
$K_m(i)$ have been certified by $\mathrm{bnfcertify}$; rows marked ``cond.'' are
not used as certified closures. The compact rows are emitted by
\texttt{condition\_c\_absorption.py} and cached as
\texttt{condition\_c\_absorption\_qmax2000\_degree40.jsonl}.

\begin{center}
\small
\begin{tabular}{r r r l r r l l}
$m$ & $q$ & $h$ & (C) & $|\overline D_B|$ & hit & cert. & defect\\
\hline
...
\begin{proposition}[The quartic $2$-adic refinement]\label{conj:Lstar}
...
The prime-quotient descent has two layers that are no longer informal
obstacles.  Condition (C), the absorbed ray criterion, is an exact global
criterion: Proposition~\ref{prop:imq} shows that
$\mathrm{Im}_q\subseteq\Cl(K_m(i))$ is essential when it is nonzero, and the
remaining class-group input is isolated in the finite residual image
$\overline D_B(m,q)\subseteq G_B/\Lambda_B$.  Condition (R), the realization
criterion, is also exact: after (N) and (C), a candidate realizes precisely when
the two reconstruction rows lie in their coefficient boxes.  The $q=185$
quotient certificate remains the model local wipeout---(C) passes, but every
survivor overflows the $S$-head box by Remark~\ref{rem:box}---although the
unconditional closure of $q=185$ used in the headline harvest is the
field-free T3 decision.
